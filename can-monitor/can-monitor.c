...
int main(int argc, char **argv) {
	...
	switch (opt) {
	case 'i':
	    strncpy(itfname, optarg, sizeof(itfname) - 1);
	    break;
	case 'r':
	...
	while (getline(&line, &size, fp) > 0) {
		...
		else if (line[0] == '!') printf(RESET "%s", line);
	}
	printf(RESET);
	return (EXIT_SUCCESS);
    }
	...
	if (isdigit(itfname[0])) {		// handle interfaces by index
		int c = itfbyindex(itfname);
		if (c < 0) {
			fprintf(stderr, "Interface index out of range\n");
			return (EXIT_FAILURE);
		}
	...
	if (live_capture) {
		handle = pcap_open_live(itfname, 1514, 1, 50, errbuf);
	    if (handle == NULL) {
	...
	close(sc);
#else
	fprintf(stderr, "Live CAN Capture not supported in this version.\n\n");
#endif
	}
    return 0;
}
