...
struct st_dc { int strt, pre, daten[8]; } dcdat, dcaltdat;

bool detail, mfxdetail, mfxread = false;
int altdauer, dcczaehler, mfxzaehler, mfxbits, paar, prea, reps, zaehler;
...
void mm_print(void)
{
	if ((mmdat.adr == mmaltdat.adr) && (mmdat.freq2 == mmaltdat.freq2) &&
		...
		if (++reps < 9) printf(" <REP>");
		else printf(" <%d>", reps + 1);
		dcaltdat.daten[0] = -1;
	}
	else {
		reps = 0;
		...
		if (paar < 16) {
			...
			for (i=0; i<=(bp/9); i++) {
				...
				if (bp/9 > i) chk ^= dcdat.daten[i];
					...
					printf("(OK)");
					if (dcdat.daten[0] != dcaltdat.daten[0]) reps = 0;
					reps++;
					dcaltdat = dcdat;
					int sm = (dcdat.daten[0] < 128) & (dcdat.daten[0] >= 112);
			...
			if (mfxdaten[0] && (mfxdaten[0] < 0x40) && mfx_crc()) printf(" CRC-Fehler ");
				...
				if (start == 0) switch (typ) {
				  ...
				  case  8: case 9:  case 10: case 11:
							printf("A07:%u ", mfxwert(start+2, 7));
							start += 9;		break;
				  case 12: case 13:
							printf("A09:%u ", mfxwert(start+3, 9));
							start += 12;	break;
				  case 14:	printf("A11:%u ", mfxwert(start+4, 11));
							start += 15;	break;
				  case 15:	printf("A14:%u ", mfxwert(start+4, 14));
							start += 18;		break;
				  ...
				  case  0:	printf("V3:%u ", mfxwert(start+4, 3));
							start += 7;		break;
				  case  1:	printf("R3:%u ", mfxwert(start+4, 3));
							start += 7;		break;
				  case  2:	printf("V7:%u ", mfxwert(start+4, 7));
							start += 11;	break;
				  case  3:	printf("R7:%u ", mfxwert(start+4, 7));
							start += 11;	break;
				  ...
				  case  7:	printf("F16:%X ", mfxwert(start+4, 16));
							start += 20;	break;
				  case  8:
				  case  9:	printf("Func %u auf %d ", mfxwert(start+3, 7), mfxdaten[start+11] & 1);
							start += 12;	break;
				  ...
				  case 14:	switch (mfxwert(start+4, 2)) {
								...
								case 1:	mfxdaten[mfxbits-8] = 0;
										...
										start += 32;
										break;
								case 2: printf("Such mit %u Bits von Id-Maske 0x%lX ",
											mfxwert(start+6, 6), mfxlwert(start+12, 32));
										start += 44;
										break;
								case 3:	printf("Neue Adresse %u fuer Decoder mit Id 0x%lX ",
				  ...
				  case 15:	switch (mfxwert(start+4, 2)) {
								...
								case 1: printf("Bake von Zentrale 0x%X, Opt 0x%X ",
										...
										start += 54;
										break;
								default:printf("Unbekannter Subtyp %u von Typ 15 ",
											mfxwert(start+4, 2));
										start = 999;
							}
							break;
